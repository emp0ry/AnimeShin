...
class RemoteModuleEntry {
  ...
  static RemoteModuleEntry? fromJson(Object? v) {
    ...
    final enabled = enabledRaw is bool
        ? enabledRaw
        : (enabledRaw is String ? enabledRaw.toLowerCase() == 'true' : true);
...
/// Manages downloaded JS modules stored on disk.
class RemoteModulesStore {
  ...
  Future<SourcesModuleDescriptor> addOrUpdateFromUrl(
    ...
    final name = (meta['sourceName'] ?? meta['name'] ?? meta['title'] ?? id).toString();
  ...
  Future<List<SourcesModuleDescriptor>> buildDescriptors({bool includeDisabled = false}) async {
    ...
    for (final e in entries) {
      ...
      try {
        meta = (jsonDecode(await jsonFile.readAsString()) as Map).cast<String, dynamic>();
      } catch (_) {
        meta = null;
      }

      final name = (meta?['sourceName'] ?? meta?['name'] ?? meta?['title'] ?? e.id).toString();
