...
class ExportButton extends ConsumerWidget {
  ...
  Future<void> _exportMal(BuildContext context, WidgetRef ref, {required bool ofAnime}) async {
    ...
    try {
      ...
      final savedPath = await saveMalXml(context, payload.filename, payload.bytes);
      if (!context.mounted) return;
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Export timeout. Check connection and try again.')),
        );
      }
    } catch (e) {
      closeDialogIfOpen();
  ...
  Future<void> _exportShiki(BuildContext context, WidgetRef ref, {required bool ofAnime}) async {
    final viewerId = ref.read(viewerIdProvider);
    if (viewerId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please sign in to export')),
    ...
    try {
      ...
      final savedPath = await saveShikiJson(context, payload.filename, payload.bytes);
      if (!context.mounted) return;
